...
/**
    ...
    - Added CONFIG_OBSTACLE_AVOIDANCE_SENSOR and CONFIG_OBASTACLE_AVOIDANCE_SENSOR_ANALOG
    - Changed small codes
    ...
    - Added HC06_bluetooth_config.ino code for changing HC-06 bluetooth baud rate from default 9600 to 115200.
    ...
    3) MPU6050 Six-Axis (Gyro + Accelerometer) MEMS MotionTracking Devices
...
#define CONFIG_OBASTACLE_AVOIDANCE_SENSOR_ANALOG
#define CONFIG_BT_SOFTWARE_SERIAL  //bluetooth software serial
//#define CONFIG_BLUETOOTH_CONFIG_WRITE_TEST //bluetooth configuration write
...
/*******************************************
    M2839 DC motor
    Motor-owned models: RS-385
    Operating voltage range: DC: 6-12v
    Rated torque: 8kgf.cm
    Deceleration ratio: 81: 1
    The whole machine weight: 145g / support
    "No-load test parameters" Tolerance ¡¾ 10%
    
    Voltage / V	No-load current (mA)	Speed (min)	Stall Current (mA)
    ------------------------------------------------------------------
    6V	        140	                    50	        1200
    9V	        155	                    75	        2000
    12V	        162	                    100	        2800
...
*******************************************/
//#define CONFIG_M2839_DC_MOTOR


//#define CONFIG_FORWARD_CONPENSATION
...
#ifdef CONFIG_OBASTACLE_AVOIDANCE_SENSOR_ANALOG
#define OBSTACLE_SENSOR_READ(pinNum) analogRead (pinNum)
#else
#define OBSTACLE_SENSOR_READ(pinNum) digitalRead(pinNum)
#endif


#define OBSTACLE_AVOIDANCE_DISTANCE_THRESHOLD   150

#define DELAY_UNIT  10 //10 ms

//255 means 5V : speed, range 0 to 255
...
int iSpeed              = MOTOR_SPEED_MAX; //speed, range 0 to 255
...
//#define BT_SOFTWARE_BAUD_RATE    9600
#define BT_SOFTWARE_BAUD_RATE    115200
...
#ifdef CONFIG_BLUETOOTH_CONFIG_WRITE_TEST
/**
...
void sendBTCommand(char * cmd1, char * cmd2) 
{
    char reply[100];
    int i = 0;
    //debugging message
...
/**
 * This function write configuration to bluetooth module
 *
 * @param       none
 * @return      none
 * @see         none
...
{
    //set BT baud rate
    btSerial.begin(BT_SOFTWARE_BAUD_RATE);

    // Now configuration start
    sendBTCommand("AT",NULL);
    // Change Name of BT: AT+NAMExxx e.g. AT+NAMEHC06
    sendBTCommand("AT+NAME",ssid); 
    // Change Baud : AT+BAUDx  e.g. AT+BAUD4
    sendBTCommand("AT+BAUD",baudios); 
    // Change Password : AT+PINxxxx  e.g. AT+PIN1234  
    sendBTCommand("AT+PIN",password); 
...
/**
 * This function updates speed to PWM.
 * 
 * @param       int speed
 * @return      it returns updated speed value.
 * @see         none
 */
...
/**
 * This function decides obstacle
 *
 * @param       int pin - pin number
 * @return      if obstacle, it return 1. Otherwise it return 0.
 * @see         none
 */

int isObstacle(int pin)
{
    int Val;
    Val = OBSTACLE_SENSOR_READ(pin);
    ...
    Serial.print("\n");
#ifdef CONFIG_OBASTACLE_AVOIDANCE_SENSOR_ANALOG
    Val = (Val < OBSTACLE_AVOIDANCE_DISTANCE_THRESHOLD) ? 1:0;
#endif
    return Val;    
}
...
int getDirectionFromdetection() 
{
    ...
    int ultra_sonic_direction = MOVE_INITIAL;
    int obstacle_sensor_direction = MOVE_INITIAL;
...
#if 0 // it is not needed in case of my case.
    //timer
    ...
    {
        stopp(10);
        ultra_sonic_direction = MOVE_BACKWARD;
    }
    ...
    if (((Fspeedd <= 20) && (Fspeedd > 10)) || (stCarInfo[PreIndex].CurrenMove == MOVE_STOP))
    {
        ...
        delay(250);
        //set distance sensor position as forward
        getDistance(ULTRASONIC_000, degreesForward, 'F');


        if (Lspeedd < 15 && Rspeedd < 15 )
        {
            ultra_sonic_direction = MOVE_BACKWARD; //go back
        }
        else
        {
            ...
            if (Lspeedd > Rspeedd)
            {
                ultra_sonic_direction = MOVE_LEFT; // go left
            }
            else
            {
                //if left distance less than right
                ultra_sonic_direction = MOVE_RIGHT; //go right
            }
        }
    }

#ifdef CONFIG_OBSTACLE_AVOIDANCE_SENSOR
    // 1 is obstacle, 0 is no obstacle
    Lavoidsensor = isObstacle(OBSTACLE_AVOIDANCE_LEFT_SENSOR_PIN);
    Ravoidsensor = isObstacle(OBSTACLE_AVOIDANCE_RIGHT_SENSOR_PIN);

    if (Lavoidsensor && Ravoidsensor)
    {
        stopp(10);
        obstacle_sensor_direction = MOVE_BACKWARD;
        goto end;
    }
    else if (Lavoidsensor && !Ravoidsensor)
    {
        stopp(10);
        obstacle_sensor_direction = MOVE_BACKWARD_RIGHT; //back and right
        goto end;
    }
    else if (!Lavoidsensor && Ravoidsensor)
    {
        stopp(10);
        obstacle_sensor_direction = MOVE_BACKWARD_LEFT; //back and left
        goto end;
    }
    ...
    */
    ...
    if (obstacle_sensor_direction != MOVE_INITIAL)
    {
        directionn = obstacle_sensor_direction;
    }
    else
    {
        directionn = ultra_sonic_direction;
    }
...
#ifdef CONFIG_FORWARD_CONPENSATION
...
*
* @param       none
* @return      if speed key is pressed, it returns true and speed value is updated.
* @see         none
*/
bool speedCommandPressed()
{
    ...
    if (irrecv.decode(&infrared) == true)
    {
        ...
        else if (strCmd == "SPEED UP")
        {
            ...
            speed += 20;
            //255 means 5V
            iSpeed = min(speed, MOTOR_SPEED_MAX);
            update_speed(iSpeed);            
...
void setup()
{
    ...
    #if defined(CONFIG_BLUETOOTH_CONFIG_WRITE_TEST)
    writeBTConfig();
    #endif
#endif
    //motor connections
...
void loop()
{
  /*test*/
    ...
    Serial.print("\n Avoidance Left: ");
    //Serial.print(OBSTACLE_SENSOR_READ(OBSTACLE_AVOIDANCE_LEFT_SENSOR_PIN));
    isObstacle(OBSTACLE_AVOIDANCE_LEFT_SENSOR_PIN);
    Serial.print("\n Avoidance Right: ");
    //Serial.print(OBSTACLE_SENSOR_READ(OBSTACLE_AVOIDANCE_RIGHT_SENSOR_PIN));
    isObstacle(OBSTACLE_AVOIDANCE_RIGHT_SENSOR_PIN);
    delay(200);
#else
